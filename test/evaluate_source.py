...
import dataset
from train.source_model import static_model
...
# evaluation
parser.add_argument('--load-epoch', type=int, default=100, help="resume trained model")
parser.add_argument('--batch-size', type=int, default=16, help="batch size")
parser.add_argument('--list-file', type=str, default='ucf101_hmdb51_val_da.txt',)
...
def set_logger(log_file='', debug_mode=False):
	...
	if not ".log" in log_file:
		...
		log_file = log_file + "-" + today + "-source-" + str(log_id) + ".log"
...
if __name__ == '__main__':
	...
	# creat model
	net_feat, net_fc, net_cls, input_config = get_source_symbol(name=args.network, segments=args.segments, 
	...
	net = static_model(net_feat=net_feat, net_fc=net_fc, net_cls=net_cls, model_prefix=args.model_prefix)
	net.load_checkpoint(epoch=args.load_epoch)

	# data iterator:
	data_root = "../dataset/{}".format(args.src_dataset)
	target_mean, target_std = input_config['mean'], input_config['std']
	if args.src_dataset == 'ARID':
