...
import dataset
from train_source_model import train_source_model
from network.source_symbol_builder import get_source_symbol

torch.backends.cudnn.enabled = True

parser = argparse.ArgumentParser(description="Video Domain Adaptation Parser")
# debug
...
parser.add_argument('--src-dataset', default='UCF101')
...
# algorithm
parser.add_argument('--network', type=str, default='TRN',help="choose the base network")
...
parser.add_argument('--lr-steps', type=list, default=[int(1e4*x) for x in [5,10,15]], help="# of samples before changing lr")
...
def autofill(args):
	...
	if not args.log_file:
		if os.path.exists("./exps/logs"):
			args.log_file = "./exps/logs/{}-{}_at-{}-source-1.log".format(args.task_name, today, socket.gethostname())
		else:
			args.log_file = ".{}-{}_at-{}-source-1.log".format(args.task_name, today, socket.gethostname())
	# fixed
...
if __name__ == "__main__":
	...
	net_feat, net_fc, net_cls, input_conf = get_source_symbol(name=args.network, pretrained=args.pretrained_2d if args.resume_epoch<0 else None, 
								segments=args.segments, consensus_type=args.consensus_type, fcbn_type=args.fcbn_type, classifier_type=args.classifier_type,
	...
	lr_expansion = round(float(train_num_dict[args.src_dataset.upper()] / base_num), 2)
	...
	train_source_model(net_feat=net_feat, net_fc=net_fc, net_cls=net_cls, **kwargs)
