...
class SegmentalSampling(object):
	def __init__(self, num_per_seg, segments=5, frame_per_seg=1, interval=1, shuffle=False, fix_cursor=False, seed=0):
		self.memory = {}
	...
	def sampling(self, range_max, v_id, prev_failed=False):
		...
		frame_per_seg = self.frame_per_seg   
		interval = self.rng.choice(self.interval)
		frame_range = (num_per_seg - 1) * interval + 1
		frame_range = min(num_per_seg, frame_range)
		...
		for seg in range(segments):
			...
			# sampling clips
			if v_id not in self.memory:
				clips = list(range(start_id, end_id, interval))
				if self.shuffle:
					self.rng.shuffle(clips)
				self.memory[v_id] = [-1, clips]
			# pickup a clip
			...
			if prev_failed or not self.fix_cursor:
				self.memory[v_id][0] = cursor
			# sampling within clip
			idxs_seg = range(clips[cursor], min(clips[cursor]+frame_range, range_max), interval)
			idxs_seg = list(idxs_seg)
			idxs_seg = np.random.choice(idxs_seg, frame_per_seg)
			for idx in idxs_seg:
				idxs.append(idx)
		return idxs
...
if __name__ == "__main__":
	...
	segmental_sampler = SegmentalSampling(num_per_seg=16, segments=5, frame_per_seg=1, interval=2, fix_cursor=False, shuffle=True)

	logging.info("SegmentalSampling():")
	for i in range(5):
		logging.info("{:d}: v_id = {}: {}".format(i, i, list(segmental_sampler.sampling(range_max=100, v_id=i))))